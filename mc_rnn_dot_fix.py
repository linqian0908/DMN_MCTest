...
class DMN:
    ...
    def __init__(self, train_raw, dev_raw, test_raw, word2vec, word_vector_size, 
        ...
        print "==> model: RNN, dot similarity, fixed embedding"
        ...
        for iter in range(1, self.memory_hops + 1):
            ...
            memory.append(self.RNN_update(memory[iter - 1], current_episode,
        ...
        for choice in [self.ca_var, self.cb_var, self.cc_var, self.cd_var]:
            history, _ = theano.scan(fn=self.input_rnn_step, 
                    sequences=choice,
                    outputs_info=T.zeros_like(self.b_inp_hid))
        ...
        self.params = [self.W_inp_hid_in, self.W_inp_hid_hid, self.b_inp_hid,
                       self.W_mem_hid_in, self.W_mem_hid_hid, self.b_mem_hid,
    ...
    def RNN_update(self, h, x, W_hid_in, W_hid_hid, b_hid):
        return T.nnet.sigmoid(T.dot(W_hid_in, x) + T.dot(W_hid_hid, h) + b_hid)
    
    def input_rnn_step(self, x, prev_h):
        return self.RNN_update(prev_h, x, self.W_inp_hid_in, self.W_inp_hid_hid, self.b_inp_hid)
    ...
    def new_episode_step(self, ct, g, prev_h):
        gru = self.RNN_update(prev_h, ct, self.W_mem_hid_in, self.W_mem_hid_hid, self.b_mem_hid)
